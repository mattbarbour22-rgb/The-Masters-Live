...
const eliminatedCount = pool.filter(p => p.eliminated).length;
  return (
    <main className="page" style={{ '--hero-image': `url(${tournamentConfig.heroImage})` }}>
      <div className="header">
        <div className="logo masters-logo"><h1>MASTERS</h1><div>AUGUSTA</div><div>LIVE</div></div>
        <div className="title">
  ...
  <div className="updated">{updatedText}</div>
      ...
      <div className="grid">
        <section className={`panel ${golfExpanded ? 'expanded' : ''}`}>
          <div className="panel-title">Masters Live Leaderboard</div>
          <table>
          ...
          <button className="footer-btn" onClick={() => setGolfExpanded(!golfExpanded)}>{golfExpanded ? 'COLLAPSE MASTERS LEADERBOARD ▲' : 'FULL MASTERS LEADERBOARD ▶'}</button>
        </section>
  ...
  {tournamentStarted && leader
    ...
    : 'Waiting for first scores. Pool leader will show once play begins.'}
      ...
      <div className="note">Live rankings are decided by best current golf position, then next best and third pick. Leaderboard standings are updated approximately every 15 minutes during the first three rounds and every 5 minutes during the final round. Results are unofficial until verified by Jonesy.
 </div>
    </main>
  );
}
